...
namespace vlmc::distance {
using bucket_t = std::vector<container::KmerPair>;
...
template <typename VC> out_t dvstar(VC &left, VC &right) {
  out_t dot_product = 0.0;
  out_t left_norm = 0.0;
  out_t right_norm = 0.0;

  auto f = [&](const ReadInKmer &left_kmer, const ReadInKmer &right_kmer) {
    dot_product +=
        ((left_kmer.next_char_prob[0] * right_kmer.next_char_prob[0]) +
    ...
    right_norm +=
        ((right_kmer.next_char_prob[0] * right_kmer.next_char_prob[0]) +
  ...
  if (left.size() < right.size()) {
    container::iterate_kmers(left, right, f);
  } else {
    container::iterate_kmers(right, left, f);
  }
...
void dvstar_kmer_major(bucket_t &left_vector, bucket_t &right_vector,
                       matrix_t &dot_prod, matrix_t &left_norm,
                       matrix_t &right_norm) {
  auto rec_fun = [&](size_t &left, size_t &right) {
    ...
    dot_prod(left_id, right_id) +=
        ((left_kmer.next_char_prob[0] * right_kmer.next_char_prob[0]) +
         (left_kmer.next_char_prob[1] * right_kmer.next_char_prob[1]) +
         (left_kmer.next_char_prob[2] * right_kmer.next_char_prob[2]) +
         (left_kmer.next_char_prob[3] * right_kmer.next_char_prob[3]));
    left_norm(left_id, right_id) +=
        ((left_kmer.next_char_prob[0] * left_kmer.next_char_prob[0]) +
         (left_kmer.next_char_prob[1] * left_kmer.next_char_prob[1]) +
         (left_kmer.next_char_prob[2] * left_kmer.next_char_prob[2]) +
         (left_kmer.next_char_prob[3] * left_kmer.next_char_prob[3]));
    right_norm(left_id, right_id) +=
        ((right_kmer.next_char_prob[0] * right_kmer.next_char_prob[0]) +
         (right_kmer.next_char_prob[1] * right_kmer.next_char_prob[1]) +
         (right_kmer.next_char_prob[2] * right_kmer.next_char_prob[2]) +
         (right_kmer.next_char_prob[3] * right_kmer.next_char_prob[3]));
  ...
  utils::matrix_recursion(0, left_vector.size(), 0, right_vector.size(),
                          rec_fun);
}
} // namespace vlmc::distance